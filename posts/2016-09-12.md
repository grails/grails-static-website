...
Since Grails<sup>&reg;</sup> 2.4.0, [asset-pipeline](https://plugins.grails.org/plugin/grails/asset-pipeline) has been the default plugin recommended to manage the static resources in a Grails application. In earlier versions of the Grails framework, the [resources](http://grails-plugins.github.io/grails-resources/) plugin was an option, but with Grails 3, the only viable option is using asset-pipeline because resources is not available in Grails 3.
