...
Beginning May 1, 2021, you will no longer be able to publish your Grails plugins/artifacts to JFrog Bintray. We have already copied most of the Grails plugins from JFrog Bintray to the [Grails Artifactory](https://repo.grails.org), which allows us to resolve those artifacts. We recommend that you start publishing artifacts to Maven Central.
...
When publishing Grails plugins to Bintray, it was possible to publish under the same groupId (i.e. org.grails.plugins) which needs to be changed while publishing to Sonatype.
 1. In case your Grails plugins are on Github, you could use “com.github.yourusername”, “com.github.yourorganization”, etc. For example: com.github.puneetbehl, where puneetbehl is my Github username.
...
### 2. Sign up and provide project details
Once you identify the groupId, you need to “Claim Your Namespace on the Central Repository.”
...
## Publishing
In order to deploy your plugin artifacts to OSSRH, you must meet the requirements for metadata in the POM and sign the artifacts. But first, let’s remove the previous Grails plugin, which is used to upload artifacts to JFrog Bintray.
...
### 2. Signing Artifacts
Use the Gradle “Signing” plugin to sign the artifacts including metadata and pom. Please check the Gradle Signing Plugin page for more details.
...
The metadata is hosted at https://github.com/grails/grails-plugins-metadata/blob/main/grails-plugins.json. When you publish a new plugin or release an existing plugin, you must send a pull request to this file to update the information on the portal. 
